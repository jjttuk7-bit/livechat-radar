...
| 2026-08-13 | 라이브 피드 좌측 세로 컬럼 재배치 | src/components/talk/LiveFeed.tsx (신규), src/App.tsx | 피드가 우측 하단 작은 상자(8줄)에 있어 채팅 흐름이 안 보였다. 3열 레이아웃(좌 피드·중 분석·우 리스크/큐)으로 바꿔 화면 높이 전체를 쓴다 — 표시 줄 수 8 → 77줄. 헤더에 실시간 CPM·수집 건수 표시 |
| 2026-08-13 | 자동 스크롤 재작성 (3번째 시도에 성공) | src/components/talk/LiveFeed.tsx | ① `behavior:'smooth'`는 3초 주기 갱신에서 애니메이션이 서로를 취소해 맨 위에 멈춤 ② `requestAnimationFrame` + cleanup의 `cancelAnimationFrame`은 다음 렌더가 프레임보다 먼저 와서 한 번도 실행되지 않음 ③ 프로그램적/사용자 스크롤을 플래그로 구분하려 했으나 스크롤 이벤트가 비동기 병합되어 순서 보장 안 됨. 최종: `useLayoutEffect`에서 동기 이동 + **간격(gap)만 기록**해 구분 자체를 없앰. 브라우저 3단계 검증(추적/위치유지/재개) 통과 |
| 2026-08-13 | 평문 500이 클라이언트를 깨뜨린 구조 결함 fix | server.ts, src/App.tsx, vercel.json | 사용자 보고: "AI 분석 통신 실패: Unexpected token 'A', \"An error o\"... is not valid JSON". `runPrefilter`와 `!ai` 폴백이 **try 밖**에 있어 거기서 던지면 Express가 평문 500을 반환하고 클라이언트 JSON 파싱이 죽으며 **원인이 사라졌다**. 두 엔드포인트 전체를 try로 감싸 어떤 경로로도 JSON만 반환. 클라이언트에 `readJson()` 도입 — 비-JSON이면 HTTP 상태와 본문 앞부분을 담아 던진다 |
| 2026-08-13 | 실시간 분석 시간 예산 명시 | server.ts, src/App.tsx, vercel.json | 실시간 분석은 40초 주기 안에 끝나야 한다. 정치 채널은 리스크·요구 후보가 폭증해 전수 포함분이 커지고 프롬프트가 무거워진다 → 실시간 경로만 `mandatoryCap 200 → 90`(리포트는 유지). `fetch`에 타임아웃 없어 요청이 멈추면 다음 주기가 영영 밀리므로 AbortController 45s(리포트 90s) 추가. `maxDuration 30 → 60` |
| 2026-08-13 | 클라이언트 CPM이 0에 고정되던 버그 | src/App.tsx | CPM 인터벌이 `[messages, peakCpm]`에 의존해 댓글이 들어올 때마다(~3초) 해제·재생성됐다. 재생성 주기가 인터벌 주기(3초)보다 짧으면 타이머가 **영영 발화하지 않는다** — 바쁜 채널일수록 0에 고정. 서버 L1은 80 CPM인데 화면은 0을 표시. 의존성 없는 단일 인터벌 + `messagesRef`로 교체 |
